...
//		this.loadedMassnahme.isValid = this.isTargetDateValid(field.getValue());
	...
	onTargetDateFocusLost: function(field) {
		...
		if(result.message) {
			if(result.isValid) {//Massnahmen Validierungsfenster f�r Warnungen sollen nur einmal kommen
	...
	isTargetDateValid: function(date) {//, skip
		...
		//kann das target nicht falsch sein. Daher keine Pr�fung, damit beim Zugriff auf die nicht existierende
...
//		grid.getStore().clearGrouping();
		grid.getStore().groupBy(this.statusWertDisplayField, true);//'statusWert'  multisort in Z. 24730 l�st mit this.fireEvent('datachanged', this); selectionModel's beforeselect event aus 
//		grid.getView().refresh();
//		grid.getStore().skipDataChanged = false;
		
		
		if(this.isLinkCiSelect) {
			//wenn updateMassnahmenTable NICHT durch user mit cbCompliantStatus Selektion ausgel�st wurde, z.B. durch Klick auf cbLinkCiList
			//muss der neue grid index der ausgew�hlten massnahme gesichert werden, damit die neuen template massnahmen
...
//		this.scrollToMassnahme();
		...
		//auskommentiert wegen: grid.getSelectionModel().getSelected() is undefined und nicht mehr n�tig da beforeselect aktiv (?)
	},
...
//	},
	...
	onSigneeApprove: function(button, event) {
		...
		var newDate = new Date(year + 1, month + 1, 0);//month + 1 weil datefield bei 1 anf�ngt
...
//			this.previousLoadedMassnahme = records[0].data;
	...
	validateMassnahmen: function(options) {
		//gerade gmachte �nderungen der aktuell ausgew�hlten Massnahme nach der letzen und vor der n�chsten Massnahmenauswahl sichern
...
//		this.existsInvalidMassnahme = true;
		...
		for(var key in this.editedMassnahmen) {
			...
			//wenn massnahme verlinkt ist keine Validierungspr�fungen, damit gespeichert werden kann
			...
			if(massnahme.statusId === 3 || massnahme.statusId === 4) {
				...
				//wenn massnahme.gapEndDate > -1 (d.h. gapEndDate nicht zur�ckgesetzt und immer wenn gapClass NICHT 4 oder 5, muss das targetDate gepr�ft werden)
				var toBeChecked = (!options || !options.skipTargetDate) && massnahme.gapEndDate > -1;
				//oder umgekehrt: option f�r die targetDate gecheckt werden muss: wenn manuell dfTargetDate ge�ndert
...
//					var isValid = this.isTargetDateValid(new Date(parseInt(massnahme.gapEndDate)), true);
					...
					if(result.isValid) {//isValid
						if(result.isWarning && !this.isWarningMassnahmenDone) {//damit keine Warnung kommt, wenn etwas anderes als das targetDate ge�ndert wurde
...
//		this.fireEvent('massnahmenWarning', warningMassnahmen);
	...
	/**
	 * Anders als urspr�nglich implementiert, soll nicht jede ung�ltige=unvollst�ndige Massnahme ZWISCHENgespeichert 
	 * werden um beliebig weiterarbeiten und sp�ter korrigieren zu k�nnen, sondern der User sofort gewarnt werden 
	 * die Massnahme entweder zu vervollst�ndigen oder alle �nderungen zu verwerfen!
	 * 
	 * Hier sind zwei vom Ansatz her zwei M�glichkeiten vorhanden dieses Ergebnis zu erreichen.
	 * 1. V O R  der Auswahl der neuen Massnahme (Klick auf eine Zeile in der Massnahmentabelle) die Pr�fung 
	 * und das Warnungsfenster �ffnen 
	 * (mit grid.getSelectionModel().on('beforerowselect', this.onBeforeMassnahmeSelect, this))
	 * Vorteil: der saubere, gew�nschte Weg
	 * Nachteil: das Fenster wird aus unbekanntem Grund hinter/unter dem Compliance Details Hauptfenster ge�ffnet
	 ...
	 * und nicht durch das event aus M�glichkeit 1 aufgerufen, wenn ung�ltige Massnahmen existieren, d.h.
	 * existsInvalidMassnahme=true.
	 * Vorteil: umgeht den Nachteil aus M�glichkeit 1
	...
	openInvalidMassnahmeWindow: function() {
		...
		/*var cancelCallback = function() {
			...
			this.selectMassnahme(grid, this.previousSelection, previousMassnahmeId);//den alten falschen/unvollst�ndigen neu laden
//			this.selectMassnahme(grid, this.previousSelection, this.previousLoadedMassnahme.itsecMassnahmenStatusId);//den alten falschen/unvollst�ndigen neu laden
			...
			//Aufrufe die nach den Ladeergebnissen kommen, �berlappen.
//			this.selectMassnahme(grid, rowIndex);//dann den neu ausgew�hlten neu laden
		};*/
...
//				grid.getSelectionModel().selectRow(this.previousSelection);//wenn beforeselect von selModel NICHT verwendet wird, muss zur�ckgesetzt werden
		};
...
//		var ident = this.getMassnahmeHeaderValueByAttr(massnahme, 'ident');
	...
	getMassnahmeHeaderValueByAttr: function(massnahme, attr) {
		...
		//wenn Massnahme verlinkt ist, ist die itsecMassnahmenStatusId nat�rlich nicht im gridStore vorhanden, daher:
		if(!r)
			r = this.getSelectedGridRecord();
...
//		if(cbLinkCiType.getValue().length > 0) {//ORIG
		if(cbLinkCiType.getValue().length > 0 && cbLinkCiList.getValue().length > 0) {
			...
			if(tableId == AC.TABLE_ID_APPLICATION)//oder generische Hilfsfunktion f�r theoretisch andere CI Typen mit Subtypen
...
//			refPKID: linkCi,
		...
		if(!this.hasNoGapAnalysis && (compliantStatusId === 3 || compliantStatusId === 4)) {
			...
			this.editedMassnahmen[rowIndex].gapEndDate = dfTargetDate.getValue() && dfTargetDate.getValue() instanceof Date ? dfTargetDate.getValue().getTime() : -1;//dfTargetDate.getValue().length > 0  -1 f�r SoapProxy, bei null statt -1 kommt JS Fehler
...
//				this.editedMassnahmen[rowIndex].signee = tfSignee.getValue();
	...
	//massnahme kann entweder die gerade aus der Tabelle ausgew�hlte sein oder die durch die combobox Link - CI ausgew�hlte.
	//Im letzteren Fall d�rfen die cbLinkCiType und cbLinkCiList nicht aktualisiert werden, da diese ja neu gesetzt wurden
	//und sonst der alte Wert f�lschlicherweise wieder zur�ckgesetzt wird! Dies ist dann der Fall wenn this.isLinkCiSelect=true.
	...
	//				var ciType = massnahme.refTableID == AC.TABLE_ID_APPLICATION ? massnahme.refCiSubTypeId : massnahme.refTableID;
		...
		if(this.isLinkCiSelect || this.existsInvalidMassnahme > 0) {// || this.getSelectedGridRecord().get('itsecMassnahmenStatusId') != massnahme.statusId wenn massnahme.statusId !=  der compliant statusId aus der Tabelle
			//zur Unterscheidung ob der Status der vorherigen falschen/unvollst�ndigen Massnahme erst dieses flag
...
//			this.updateGapRelevance(massnahme.statusId, gapClassId);
		...
		if(this.isLinkCiSelect) {
			//hier muss gespeichert werden, wenn durch skipLinkCiValidation: true direkt nach der �nderung der Einzelverlinkung
			//nicht sofort gespeichert werden soll! Siehe onLinkCiSelect()
			this.onMassnahmeChange();//wird durch klick auf andere massnahme in massnahmen grid/tabelle ausgel�st
	...
	onRiskAnalysisTypeCheck: function(checkbox, isChecked) {
		...
		//unterscheiden. Daher pr�fen ob ein reload stattgefunden hat. Entweder durch eine zuvor nicht gew�hlt
...
//		this.isWarningMassnahmenDone = false;
		...
		//mit onFocusLost) �nderungen des targetDate und die Anpassung der gapClass. 
		//Aber nicht umgekehrt wenn die gapClass ge�ndert wird! Sie w�rde aber ohne den skipTargetDate option Parameter
		//in beiden F�llen durch/nach onMassnahmeChange() aufgerufen. Daher der options Parameter skipTargetDate f�r 
...
//		var massnahme = this.editedMassnahmen[this.getSelectedGridIndex()];
	...
	onMassnahmenWarning: function(warningMassnahmen) {
		...
		//Massnahmen Validierungsfenster f�r Warnungen sollen nur einmal kommen
...
//					this.existsInvalidMassnahme = 0;
					//wenn von compliantStatus 4,5 auf 1,2,3 gewechselt wird, k�nnen die Fehler unausgef�llten
					//Felder ignoriert werden. Dies erzeugt jedoch einen Widerspruch, wenn andere Massnahmen mit
					//compliantStatus=1,2,3 ausgew�hlt werden. Dann sollte this.existsInvalidMassnahme nicht 0
					//gesetzt werden.
...
//			items[i].reset();
		...
		for(var i = 0; i < items.length; i++) {
//			items[i].reset();//(*checkbox1) oder manuell zur�cksetzen, damit kein check event gefeuert wird
	...
	setTargetDate: function(gapClassId) {
		...
		switch(gapClassId) {
			case '1'://long-term
				year++;
				month++;
				//sonst wird vom datefield ein Monat zu fr�h gesetzt wenn nur das jahr erh�ht wird.
				//f�r das datefield ist im Gegensatz zum Date() der erste Monat die 1
...
//				this.openMassnahmeValidationWindow(result.gapClass, result.title, result.iconType, result.message, result.isValid);
			...
			case '2'://mid-term
				...
				var oldDate = dfTargetDate.getValue();
				var newDate = new Date(year, month, 0);//0 f�r den letzten Tag des Monats
...
//		return isValid;
	...
	getSigneeValueByCwid: function(combo, cwid) {
		...
		//falls r undefined Warnmeldung auf Statusbar, dass signee cwid zu dieser Massnahme nicht mehr g�ltig ist, da nicht
...
//		var userName = combo.getValue();
	...
	//		this.onMassnahmeChange();//ORIG
		...
		//werden! Deshalb soll die Validierung durch Ver�nderung der Einzelverlinkung der Massnahme nicht durchgef�hrt
		//werden, weil dies fachlich auch nicht notwendig ist. Die Massnahme auf die die ausgew�hlte Massnahme verweisen soll
		//ist zwangsl�ufig valide, sonst h�tte sie nicht irgendwann gesichert werden k�nnen.
//		var options = {
//			skipLinkCiValidation: true
//		};
//		this.onMassnahmeChange(options);
