...
//		this.getComponent('cbLinkCiType').getStore().filter('language', AAM.getLanguage().toLowerCase());
	...
	loadLinkCiList: function(ciTypeId, callback) {
		//hier schon zur Vermeidung von Fehlerf�llen wenn Link CI-Typ ausgew�hlt, Massnahme auf unbearbeitet
		//zur�cksetzen 
...
//		this.linkCiSelected = callback ? false : true;
	...
	onLinkCiListChange: function(combo, newValue, oldValue) {
		...
		Problem wenn this.linkCiListSelected(newValue); zweimal ausgef�hrt wird durch select und change wenn combo mittels
		select bedient wird. Durch das geworfene linkCiSelect event wird updateComplianceDetails und updateMassnahmenTable
		zweimal ausgef�hrt, was zu falschen/irritierenden compliant Status �nderungen in der Massnahmentabelle. 
		Daher das change erstmal deaktiviert. Evtl. ein flag nutzen um beides zu unterst�tzen. Siehe if(this.isLinkCiSelect)
