...
//		"ORDER BY EVT.History_Event_Id, DTL.History_Detail_Id";
	...
	private static final String SQL_HISTORY_LIST = "SELECT SortKey, DateValue, CwId, ChangeUser, ChangeSource, Table_Id, Ci_Id, Object_Name, Table_Original, Table_Readable, Table_Type, Transbase_Attribute, Attribute_Glossary, Old_Value_Original, New_Value_Original, Old_Value, New_Value FROM TABLE(PCK_HISTORY.READ(:Table_Id, :Table_PK_ID, :Lang, 'N', 'Y')) ORDER BY SortKey DESC, Table_Readable, Object_Name, Attribute_Glossary, Transbase_Attribute DESC";
...
//					listResult.add(new HistorySISecViewDataDTO((Long) (((BigDecimal)obj[0]).longValue()), (String) obj[1], (String)obj[2], ((Character)obj[3]).toString(), (((BigDecimal)obj[4]).longValue()), (String)obj[5], (String)obj[6], (String)obj[7], new Integer((Character)obj[8]).intValue()));
					...
					String userName = getUserName((String) obj[3]);
...
//	}
	...
	private static String getUserName(String userID)
	{
		String result = null;
		Session session = HibernateUtil.getSession();
		Query selectQuery = session.createSQLQuery("SELECT Vorname || ' ' || Nachname || ' (' || Cwid || ')' AS \"Name\" FROM PERSON WHERE Cwid = :userID")
		.setString("userID", userID);
		try 
		{
