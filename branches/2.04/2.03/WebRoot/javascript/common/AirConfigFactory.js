	...
	return {
		...
		createAssetManagementGridConfig: function(selModel) {
		    ...
		    if(columns == undefined || columns.length == 0)
	    	{
				...
				var index = itsecUserOptionListStore.findExact('itsecUserOptionName', 'AIR_USER_PROFILE_COLUMNS_PREFERENCE');
				if(index >= 0)
				{
					columns = itsecUserOptionListStore.getAt(index).get('itsecUserOptionValue');
				}
				else
				{
