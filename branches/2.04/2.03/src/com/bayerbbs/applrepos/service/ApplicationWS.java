...
public class ApplicationWS {
	...
	public CiItemsResultDTO checkAllowedApplicationName(
			ApplicationParameterInput input) {
		String searchname = input.getQuery();
		List<CiItemDTO> listAnwendungen = null;// ApplicationDTO
		listAnwendungen = CiEntitiesHbn.findExistantCisByNameOrAlias(
				searchname, false);
		...
		if (0 != listAnwendungen.size()) {
			output.setInformationText(listAnwendungen.get(0)
					.getApplicationCat1Txt());
		}
	...
	public CiItemsResultDTO findApplications(ApplicationSearchParamsDTO input) {// ApplicationParameterInput
		...
		if (LDAPAuthWS.isLoginValid(input.getCwid(), input.getToken())) {
			...
			if (null != input.getShowDeleted()
					&& AirKonstanten.YES_SHORT.equals(input.getShowDeleted())) {
				showDeleted = true;
			}

			boolean onlyApplications = false;
			if (null != input.getIsOnlyApplications()
					&& AirKonstanten.STRING_TRUE.equals(input
							.getIsOnlyApplications())) {// getOnlyapplications
			...
			if (AirKonstanten.SEARCH_TYPE_OU_SEARCH.equals(searchAction)) {
				// String ciType = input.getCiTypeId() != null ?
				// Integer.toString(input.getCiTypeId()) :
				// AirKonstanten.STRING_EMPTY;
				String ouUnit = input.getOuUnit(); // "BBS-ITO-SOL-BPS-SEeB"
				String ciOwnerType = input.getCiOwnerType(); // "APP", "CI" oder
																// "ALL"
				String ouQueryMode = input.getOuQueryMode(); // "EXAKT" oder
																// "START"
				...
				listAnwendungen = CiEntitiesHbn.findCisByOUunit(ouCiType,
						ouUnit, ciOwnerType, ouQueryMode, input.getSort(),
						input.getDir());// ciType
			} else if (AirKonstanten.MY_CIS.equals(searchAction)) {
				if (StringUtils.isNotNullOrEmpty(cwid)) {
					listAnwendungen = CiEntitiesHbn.findMyCisOwner(cwid,
							input.getSort(), input.getDir(), onlyApplications);
				}
			} else if (AirKonstanten.MY_CIS_SUBSTITUTE.equals(searchAction)) {
				if (StringUtils.isNotNullOrEmpty(cwid)) {
					listAnwendungen = CiEntitiesHbn.findMyCisDelegate(cwid,
							input.getSort(), input.getDir(), onlyApplications);
				}
			} else if (AirKonstanten.MY_CIS_FOR_DELETE.equals(searchAction)) {
				if (StringUtils.isNotNullOrEmpty(cwid)) {
					listAnwendungen = CiEntitiesHbn.findMyCisForDelete(cwid,
							input.getSort(), input.getDir(), onlyApplications,
							input.getCiNameAliasQuery());
				}
			} else {
				if (AirKonstanten.STRING_TRUE.equals(input.getIsAdvSearch())) {
					listAnwendungen = AnwendungHbn.findApplications(searchname,
							showDeleted, input.getQueryMode(),
							input.getAppOwner(), input.getAppOwnerHidden(),
							input.getAppOwnerDelegate(),
							input.getAppOwnerDelegateHidden(),
							input.getCiOwner(), input.getCiOwnerHidden(),
							input.getCiOwnerDelegate(),
							input.getCiOwnerDelegateHidden(), onlyApplications,
							input.getSort(), input.getDir(),
							input.getCiTypeId(), input.getCiSubTypeId(),
							input.getDescription(),
							input.getOperationalStatusId(),
							input.getApplicationCat2Id(),
							input.getLifecycleStatusId(), input.getProcessId(),
							input.getIsTemplate(), input.getAppSteward(),
							input.getAppStewardHidden(),
							input.getBarRelevance(),
							input.getOrganisationalScope(), input.getItSetId(),
							input.getItSecGroupId(), input.getSource(),
							input.getBusinessEssentialId(),
							input.getCiTypeOptions(), input.getItSetOptions(),
							input.getDescriptionOptions(),
							input.getAppOwnerOptions(),
							input.getAppOwnerDelegateOptions(),
							input.getAppStewardOptions(),
							input.getCiOwnerOptions(),
							input.getCiOwnerDelegateOptions(),
							input.getGeneralUsageOptions(),
							input.getItCategoryOptions(),
							input.getLifecycleStatusOptions(),
							input.getOrganisationalScopeOptions(),
							input.getItSecGroupOptions(),
							input.getProcessOptions(),
							input.getSourceOptions(),
							input.getBusinessEssentialOptions());
				} else {
					// boolean showDeleted = true;

					listAnwendungen = CiEntitiesHbn.findCisByNameOrAlias(
							searchname, showDeleted, input.getQueryMode(),
							onlyApplications, input.getSort(), input.getDir(),
							startwert, limit);
					anzahlDatensaetze = 0L;
					// Zahl der Datensätze nur direkt in der Datenbank
					// bestimmen, wenn wir (noch) nicht geblättert haben oder
					// das Resultset > als "limit" ist.
					if (null != listAnwendungen) {
						anzahlDatensaetze = (long) listAnwendungen.size();
					}
					if (startwert != 0L || (long) limit == anzahlDatensaetze) {
						anzahlDatensaetze = CiEntitiesHbn
								.findCountCisByNameOrAlias(searchname,
										showDeleted, input.getQueryMode(),
										onlyApplications);
					}
				}
				...
				try {
					...
					CiSupportStuffHbn
							.saveCiSupportStuffAll(
									...
									dto.getCiSupportStuffApplicationDocumentation(),
									dto.getCiSupportStuffRootDirectory(), dto
											.getCiSupportStuffDataDirectory(),
									dto.getCiSupportStuffProvidedServices(),
									dto.getCiSupportStuffProvidedMachineUsers());

					if (null != dto.getLicenseUsingRegions()) {
						ApplicationRegionHbn.saveApplicationRegionsAll(
								editInput.getCwid(), dto.getId(),
								dto.getLicenseUsingRegions());
					}

					ApplicationProcessHbn.saveApplicationProcessAll(
							editInput.getCwid(), dto.getId(),
							dto.getBusinessProcessHidden());
					...
					for (String[] grouptype : AirKonstanten.GPSCGROUP_MAPPING) {
						...
						String gpscContact = (String) ApplicationDTO.class
								.getMethod(method).invoke(dto);
						String methodHidden = "get" + new String(d)
								+ AirKonstanten.GPSCGROUP_HIDDEN_DESCRIPTOR;
						String gpscContactHidden = (String) ApplicationDTO.class
								.getMethod(methodHidden).invoke(dto);
						if (!(AirKonstanten.GPSCGROUP_DISABLED_MARKER
								.equals(gpscContact))
								&& !(AirKonstanten.GPSCGROUP_DISABLED_MARKER
										.equals(gpscContactHidden))) {
							if ("Y".equals(grouptype[2])) { // Individual
															// Contact(s)
								CiPersonsHbn.saveCiPerson(editInput.getCwid(),
										dto.getTableId(), dto.getId(),
										new Long(grouptype[0]), grouptype[3],
										gpscContactHidden);
							} else { // Group(s)
					...
					if (dto.getUpStreamAdd() != null
						...
						CiEntitiesHbn.saveCiRelations(editInput.getTableId(),
								dto.getId(), dto.getUpStreamAdd(),
								dto.getUpStreamDelete(), "UPSTREAM",
								editInput.getCwid());
					...
					if (dto.getDownStreamAdd() != null
						...
						CiEntitiesHbn.saveCiRelations(editInput.getTableId(),
								...
								dto.getDownStreamDelete(), "DOWNSTREAM",
								editInput.getCwid());
					...
					//
					// ApplicationApplicationHbn.saveApplicationApplicationAll(cwid,
					// null, applicationId, dto.getUpStreamAdd(), "ADD");
					// ApplicationApplicationHbn.saveApplicationApplicationAll(cwid,
					// null, applicationId, dto.getUpStreamDelete(), "DELETE");
					// ApplicationApplicationHbn.saveApplicationApplicationAll(cwid,
					// applicationId, null, dto.getDownStreamAdd(), "ADD");
					// ApplicationApplicationHbn.saveApplicationApplicationAll(cwid,
					// applicationId, null, dto.getDownStreamDelete(),
					// "DELETE");
	...
	public ApplicationEditParameterOutput createApplication(
		...
		if (null != editInput) {
			...
			if (null == dto.getCiOwner() || dto.getCiOwner().length() == 0) {// getResponsible
				dto.setCiOwner(editInput.getCwid().toUpperCase());// oder
																	// dto.getApplicationOwner()
																	// ?
				dto.setCiOwnerHidden(editInput.getCwid().toUpperCase());// oder
																		// dto.getApplicationOwnerHidden()
																		// ?
			}
			...
			output = AnwendungHbn.createAnwendung(editInput.getCwid(), dto,
					editInput.getForceOverride(), true);

			if (AirKonstanten.RESULT_OK.equals(output.getResult())) {
				// get detail
				List<Application> listAnwendung = AnwendungHbn
						.findApplicationByName(editInput.getName());
				if (null != listAnwendung && 1 == listAnwendung.size()) {
					output.setApplicationId(listAnwendung.get(0)
							.getApplicationId());
					...
					if (null != editInput.getGpsccontactSupportGroupHidden()) {
						CiGroupsHbn.saveCiGroup(editInput.getCwid(),
								dto.getTableId(), ciId, new Long(1),
								"SUPPORT GROUP - IM RESOLVER",
								dto.getGpsccontactSupportGroup());// getGpsccontactSupportGroupHidden
					}
					if (null != editInput
							.getGpsccontactOwningBusinessGroupHidden()) {
						CiGroupsHbn.saveCiGroup(editInput.getCwid(),
								dto.getTableId(), ciId, new Long(6),
				...
				if (null != output.getMessages()
						&& output.getMessages().length > 0) {
	...
	public CiEntityEditParameterOutput deleteApplication(
			CiEntityParameterInput editInput) {// ApplicationEditParameterInput
		CiEntityEditParameterOutput output = new CiEntityEditParameterOutput();

		if (null != editInput) {
			if (LDAPAuthWS.isLoginValid(editInput.getCwid(),
					editInput.getToken())) {

				AnwendungHbn.deleteApplicationApplication(editInput.getCwid(),
						editInput.getCiId());
				AnwendungHbn.deleteApplicationItSystem(editInput.getCwid(),
						editInput.getCiId());

				output = AnwendungHbn.deleteAnwendung(editInput.getCwid(),
						editInput.getCiId());
			} else {
	...
	// deleteApplication(ApplicationEditParameterInput editInput) {
	// ApplicationEditParameterOutput output = new
	// ApplicationEditParameterOutput();
	//
	// if (null != editInput) {
	//
	 ...
	 * @return
	 */
	private ApplicationDTO getApplicationDTOFromEditInput(
			ApplicationEditParameterInput editInput) {
		...
		dto.setApplicationOwnerDelegateHidden(editInput
				.getApplicationOwnerDelegateHidden());

		dto.setGpsccontactSupportGroupHidden(editInput
				.getGpsccontactSupportGroupHidden());
		dto.setGpsccontactChangeTeamHidden(editInput
				.getGpsccontactChangeTeamHidden());
		dto.setGpsccontactServiceCoordinatorHidden(editInput
				.getGpsccontactServiceCoordinatorHidden());
		dto.setGpsccontactEscalationHidden(editInput
				.getGpsccontactEscalationHidden());
		dto.setGpsccontactCiOwnerHidden(editInput.getGpsccontactCiOwnerHidden());
		dto.setGpsccontactOwningBusinessGroupHidden(editInput
				.getGpsccontactOwningBusinessGroupHidden());
		dto.setGpsccontactImplementationTeamHidden(editInput
				.getGpsccontactImplementationTeamHidden());
		dto.setGpsccontactServiceCoordinatorIndivHidden(editInput
				.getGpsccontactServiceCoordinatorIndivHidden());
		dto.setGpsccontactEscalationIndivHidden(editInput
				.getGpsccontactEscalationIndivHidden());
		dto.setGpsccontactResponsibleAtCustomerSideHidden(editInput
				.getGpsccontactResponsibleAtCustomerSideHidden());
		dto.setGpsccontactSystemResponsibleHidden(editInput
				.getGpsccontactSystemResponsibleHidden());
		dto.setGpsccontactImpactedBusinessHidden(editInput
				.getGpsccontactImpactedBusinessHidden());
		dto.setGpsccontactBusinessOwnerRepresentativeHidden(editInput
				.getGpsccontactBusinessOwnerRepresentativeHidden());
		...
		dto.setGpsccontactServiceCoordinator(editInput
				.getGpsccontactServiceCoordinator());
		dto.setGpsccontactEscalation(editInput.getGpsccontactEscalation());
		dto.setGpsccontactCiOwner(editInput.getGpsccontactCiOwner());
		dto.setGpsccontactOwningBusinessGroup(editInput
				.getGpsccontactOwningBusinessGroup());
		dto.setGpsccontactImplementationTeam(editInput
				.getGpsccontactImplementationTeam());
		dto.setGpsccontactServiceCoordinatorIndiv(editInput
				.getGpsccontactServiceCoordinatorIndiv());
		dto.setGpsccontactEscalationIndiv(editInput
				.getGpsccontactEscalationIndiv());
		dto.setGpsccontactResponsibleAtCustomerSide(editInput
				.getGpsccontactResponsibleAtCustomerSide());
		dto.setGpsccontactSystemResponsible(editInput
				.getGpsccontactSystemResponsible());
		dto.setGpsccontactImpactedBusiness(editInput
				.getGpsccontactImpactedBusiness());
		dto.setGpsccontactBusinessOwnerRepresentative(editInput
				.getGpsccontactBusinessOwnerRepresentative());
		...
		dto.setAccessingUserCountMeasured(editInput
				.getAccessingUserCountMeasured());
		...
		dto.setCiSupportStuffUserAuthorizationProcess(editInput
				.getCiSupportStuffUserAuthorizationProcess());
		dto.setCiSupportStuffChangeManagementSupportedByTool(editInput
				.getCiSupportStuffChangeManagementSupportedByTool());
		dto.setCiSupportStuffUserManagementProcess(editInput
				.getCiSupportStuffUserManagementProcess());
		dto.setCiSupportStuffApplicationDocumentation(editInput
				.getCiSupportStuffApplicationDocumentation());
		dto.setCiSupportStuffRootDirectory(editInput
				.getCiSupportStuffRootDirectory());
		dto.setCiSupportStuffDataDirectory(editInput
				.getCiSupportStuffDataDirectory());
		dto.setCiSupportStuffProvidedServices(editInput
				.getCiSupportStuffProvidedServices());
		dto.setCiSupportStuffProvidedMachineUsers(editInput
				.getCiSupportStuffProvidedMachineUsers());
	...
	public ApplicationViewdataOutput getApplicationUpstream(
		...
		if (LDAPAuthWS.isLoginValid(detailInput.getCwid(),
				detailInput.getToken())) {
			if (null != detailInput.getId()) {
				List<ViewDataDTO> listDTO = AnwendungHbn
						.findApplicationUpStream(detailInput.getId());
	...
	public ApplicationViewdataOutput getApplicationDownstream(
		...
		if (LDAPAuthWS.isLoginValid(detailInput.getCwid(),
			...
			if (null != detailInput.getId()) {
				List<ViewDataDTO> listDTO = AnwendungHbn
						.findApplicationDownStream(detailInput.getId());
	...
	public ApplicationViewdataOutput getApplicationProcess(
		...
		if (LDAPAuthWS.isLoginValid(detailInput.getCwid(),
			...
			if (null != detailInput.getId()) {
				List<ViewDataDTO> listDTO = AnwendungHbn
						.findApplicationProcess(detailInput.getId());

				// List<ViewDataDTO> listDTO =
				// AnwendungHbn.findApplicationConnections(detailInput.getId());
	...
	public ApplicationViewdataOutput getAllConnections(
		...
		if (LDAPAuthWS.isLoginValid(detailInput.getCwid(),
			...
			if (null != detailInput.getId()) {
				List<ViewDataDTO> listDTO = AnwendungHbn
						.findApplicationConnections(detailInput.getId());
	...
	public ApplicationViewdataOutput getApplicationItSystems(
		...
		if (LDAPAuthWS.isLoginValid(detailInput.getCwid(),
			...
			if (null != detailInput.getId()) {
				List<ViewDataDTO> listDTO = AnwendungHbn
						.findApplicationItSystems(detailInput.getId());
				output.setViewdataDTO(getViewDataArray(listDTO));
			}
		}
	...
	public ApplicationEditParameterOutput createApplicationByCopy(
		...
		if (LDAPAuthWS.isLoginValid(copyInput.getCwid(), copyInput.getToken())) {
			...
			case AirKonstanten.TABLE_ID_APPLICATION:
				ApplicationDTO dto = new ApplicationDTO();
				Application applicationSource = AnwendungHbn
						.findApplicationById(copyInput.getCiIdSource());
				...
				if (null != applicationSource) {
					...
					dto.setCiOwnerDelegate(applicationSource
							.getSubResponsible());
					dto.setCiOwnerDelegateHidden(applicationSource
							.getSubResponsible());
					...
					ApplicationEditParameterOutput createOutput = AnwendungHbn
							.createAnwendung(copyInput.getCwid(), dto, null,
									neuanlage);

					if (AirKonstanten.RESULT_OK
							.equals(createOutput.getResult())) {
						List<Application> listAnwendung = AnwendungHbn
								.findApplicationByName(copyInput
										.getCiNameTarget());
						if (null != listAnwendung && 1 == listAnwendung.size()) {
							...
							Long ciId = listAnwendung.get(0).getApplicationId();
							Application applicationTarget = AnwendungHbn
									.findApplicationById(ciId);

							if (null != applicationTarget) {
								ApplicationEditParameterOutput temp = AnwendungHbn
										.copyApplication(copyInput.getCwid(),
												applicationSource.getId(),
												applicationTarget.getId(),
												copyInput.getCiNameTarget(),
												copyInput.getCiAliasTarget());

								output.setApplicationId(temp.getApplicationId());
								output.setResult(temp.getResult());
								output.setMessages(temp.getMessages());
								output.setDisplayMessage(temp
										.getDisplayMessage());
							}
						} else {
						...
						output.setDisplayMessage(createOutput
								.getDisplayMessage());
					}
				}
				break;
			...
			case AirKonstanten.TABLE_ID_BUILDING_AREA:
				BuildingWS.createBuildingAreaByCopyInternal(ciCopyInput,
						outputCI);
				break;
			...
			if (AirKonstanten.TABLE_ID_APPLICATION != copyInput
					.getTableIdSource().intValue()) {
				// setzen der Rückgabewerte von CI's in das applicationOutput
				output.setResult(outputCI.getResult());

				output.setDisplayMessage(outputCI.getDisplayMessage()); // one
																		// message,
																		// that
																		// should
																		// be
	...
	public ApplicationDetailParameterOutput getApplication(
		...
		if (LDAPAuthWS.isLoginValid(detailInput.getCwid(),
			...
			Application application = AnwendungHbn
					.findApplicationById(detailInput.getId());
			...
			applicationDTO.setApplicationCat2Id(application
	...
	public ApplicationDetailParameterOutput getApplicationDetail(
		...
		if (LDAPAuthWS.isLoginValid(detailInput.getCwid(),
			...
			if (null != application) {
				...
				if (null != dto.getTemplate() && !"0".equals(dto.getTemplate())) {
					...
					if (!"0".equals(ApplReposHbn
							.getCountReferencingTemplates(detailInput.getId()))) {
						dto.setTemplateReferencedByItem(AirKonstanten.YES_SHORT);
					}
					// check this CI is if template then go for is related with
					// other CI or not
					if ("Y".equals(CiEntitiesHbn.findCIisLinkWithTemplate(
							detailInput.getId(),
				...
				if (StringUtils.isNotNullOrEmpty(dto.getCiOwner())) {
					List<PersonsDTO> listPers = PersonsHbn.findPersonByCWID(dto
							.getCiOwner());
				...
				if (StringUtils.isNotNullOrEmpty(dto
						.getApplicationOwnerDelegate())) {
					List<PersonsDTO> listPers = PersonsHbn.findPersonByCWID(dto
							.getApplicationOwnerDelegate());
					if (null != listPers && 1 == listPers.size()) {
						PersonsDTO tempPers = listPers.get(0);
						dto.setApplicationOwnerDelegate(tempPers
								.getDisplayNameFull());
					}
				}
				...
				// hier wird geprüft, ob der aktive Anwender über
				// Edit-Zugriffsrechte (über Person, seine Gruppenzugehörigkeit
				// oder globale Adminrechte) verfügt
				if (checker.isEditable(application.getApplicationId(),
						new Long(2), detailInput.getCwid())) {
					dto.setIsEditable(AirKonstanten.YES_SHORT);
				}

				// RFC 7465
				if (checker.isRelevanceOperational(detailInput.getCwid(),
						detailInput.getToken(), dto.getApplicationCat1Id(),
						application)) {
				...
				if (AirKonstanten.APPLICATION_CAT1_APPLICATION.longValue() == applCat1Id) {
					...
					if (checker.isRelevanceStrategic(detailInput.getCwid(),
							detailInput.getToken(), dto.getApplicationCat1Id(),
							application)) {
						accessDTO
								.setRelevanceStrategic(AirKonstanten.YES_SHORT);
					} else {
						accessDTO.setRelevanceStrategic(AirKonstanten.NO_SHORT);
					}
				} else if (AirKonstanten.APPLICATION_CAT1_COMMON_SERVICE
						.longValue() == applCat1Id
						|| AirKonstanten.APPLICATION_CAT1_COMMON_MIDDLEWARE
								.longValue() == applCat1Id
						|| AirKonstanten.APPLICATION_CAT1_COMMON_APPLICATIONPLATFORM
								.longValue() == applCat1Id) {

					if (AirKonstanten.NO_SHORT.equals(accessDTO
							.getRelevanceOperational())) {
						// Abfrage Infrastructure Manager
						// if
						// (checker.isEditableRoleInfrastructureManager(detailInput.getCwid()))
						// {
						// accessDTO.setRelevanceOperational(AirKonstanten.YES_SHORT);
						// }
						if (checker.isRelevanceOperational(
								detailInput.getCwid(), detailInput.getToken(),
								dto.getApplicationCat1Id(), application)) {
							accessDTO
									.setRelevanceOperational(AirKonstanten.YES_SHORT);
						}
					}

					// diese CI's haben keine Trennung zwischen Operational und
					// Strategic
					// deshalb setzen wir hier beide Berechtigungen gleich
					accessDTO.setRelevanceStrategic(accessDTO
							.getRelevanceOperational());
				} else {
				...
				if (StringUtils.isNotNullOrEmpty(application.getInsertQuelle())) {
					if (AirKonstanten.YES_SHORT.equals(dto.getIsEditable())) {
						...
						if (AirKonstanten.INSERT_QUELLE_SISEC
								.equals(application.getInsertQuelle())
								|| AirKonstanten.APPLICATION_GUI_NAME
										.equals(application.getInsertQuelle())) {
							// for the application itself, all are editable
							accessDTO.setAllEditable();
						} else if (AirKonstanten.INSERT_QUELLE_GSDB
								.equals(application.getInsertQuelle())
								&& AirKonstanten.SERVICE_ENVIRONMENT_OWNER_SE_BCBS
										.equals(application
												.getServiceEnvironmentOwner())) {
							// GSDB and SE_BCBS is not editable RFC 9176
							accessDTO
									.setRelevanceOperational(AirKonstanten.NO_SHORT);
							accessDTO
									.setRelevanceStrategic(AirKonstanten.NO_SHORT);
							dto.setIsEditable(AirKonstanten.NO_SHORT);
							dto.setMessageText("editInGPSC");
						} else {
							InterfacesDTO interfaceDto = InterfacesHbn
									.findInterfacesByInterfaceToken(application
											.getInsertQuelle());

							if (StringUtils.isNotNullOrEmpty(interfaceDto
									.getSisecEditable())
									&& null != interfaceDto) {
								String allRights = interfaceDto
										.getSisecEditable();
								if (-1 != allRights.indexOf("License_Scanning")) {
									accessDTO
											.setLicense_Scanning(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Responsible")) {
									accessDTO
											.setResponsible(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Sub_Responsible")) {
									accessDTO
											.setSub_Responsible(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Relevance_Ics")) {
									accessDTO
											.setRelevance_Ics(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Relevanz_Itsec")) {
									accessDTO
											.setRelevanz_Itsec(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Gxp_Flag")) {
									accessDTO
											.setGxp_Flag(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Itsec_Gruppe_Id")) {
									accessDTO
											.setItsec_Gruppe_Id(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Sample_Test_Date")) {
									accessDTO
											.setSample_Test_Date(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Sample_Test_Result")) {
									accessDTO
											.setSample_Test_Result(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights.indexOf("Sla_Id")) {
									accessDTO
											.setSla_Id(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Service_Contract_Id")) {
									accessDTO
											.setService_Contract_Id(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Priority_Level_Id")) {
									accessDTO
											.setPriority_Level_Id(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Severity_Level_Id")) {
									accessDTO
											.setSeverity_Level_Id(AirKonstanten.YES_SHORT);
								}

								// business essential only by group right

								if (-1 != allRights
										.indexOf("Itsec_SB_Integ_ID")) {
									accessDTO
											.setItsec_SB_Integ_ID(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Itsec_SB_Integ_Txt")) {
									accessDTO
											.setItsec_SB_Integ_Txt(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Itsec_SB_Verfg_ID")) {
									accessDTO
											.setItsec_SB_Verfg_ID(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Itsec_SB_Verfg_Txt")) {
									accessDTO
											.setItsec_SB_Verfg_Txt(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Itsec_SB_Vertr_ID")) {
									accessDTO
											.setItsec_SB_Vertr_ID(AirKonstanten.YES_SHORT);
								}
								if (-1 != allRights
										.indexOf("Itsec_SB_Vertr_Txt")) {
									accessDTO
											.setItsec_SB_Vertr_Txt(AirKonstanten.YES_SHORT);
								}
							}
						}
					}
				...
				if (null != listApplicationRegion
						&& !listApplicationRegion.isEmpty()) {
			...
			String count = ApplReposHbn.getCountFromRoleNameAndCwid(
					AirKonstanten.ROLE_BUSINESS_ESSENTIAL_EDITOR,
					detailInput.getCwid());
			...
			if (null != application
					&& StringUtils.isNotNullOrEmpty(application
							.getDeleteQuelle())) {
				// der Datensatz ist löschmarkiert und darf nicht editiert
				// werden!
		...
		if (null != dto
				&& ((null == dto.getBarRelevance() || "N".equals(dto
						.getBarRelevance())
						&& null != dto.getBarApplicationId()))) {
	...
	private void readAndFillCiStuff(ApplicationDTO dto, Application application) {
		...
		CiSupportStuffDTO supportStuffDTO = CiSupportStuffHbn
				.findCiSupportStuffByTableAndCiAndTypeId(
						AirKonstanten.TABLE_ID_APPLICATION,
						ciId,
						AirKonstanten.CI_SUPPORT_STUFF_TYPE_UserAuthorizationSupportedByDocumentation);
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffUserAuthorizationSupportedByDocumentation(supportStuffDTO
					.getCiSupportStuffValue());
		}

		supportStuffDTO = CiSupportStuffHbn
				.findCiSupportStuffByTableAndCiAndTypeId(
						AirKonstanten.TABLE_ID_APPLICATION,
						ciId,
						AirKonstanten.CI_SUPPORT_STUFF_TYPE_UserAuthorizationProcess);
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffUserAuthorizationProcess(supportStuffDTO
					.getCiSupportStuffValue());
		}
		...
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffChangeManagementSupportedByTool(supportStuffDTO
					.getCiSupportStuffValue());
		}
		...
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffUserManagementProcess(supportStuffDTO
					.getCiSupportStuffValue());
		}
		...
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffApplicationDocumentation(supportStuffDTO
					.getCiSupportStuffValue());
		}
		...
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffRootDirectory(supportStuffDTO
					.getCiSupportStuffValue());
		}
		...
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffDataDirectory(supportStuffDTO
					.getCiSupportStuffValue());
		}
		...
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffProvidedServices(supportStuffDTO
					.getCiSupportStuffValue());
		}
		...
		if (null != supportStuffDTO) {
			dto.setCiSupportStuffProvidedMachineUsers(supportStuffDTO
					.getCiSupportStuffValue());
		}
	...
	public ApplicationContactsParameterOutput getApplicationContacts(
			ApplicationContactsParameterInput input) {
		...
		List<ApplicationContact> listContacts = AnwendungHbn
				.findApplicationContacts(input.getApplicationId(),
						input.getTableId());
		...
		while (itContacts.hasNext()) {
			...
			if (!lastGroupTypeName.equals(contact.getGroupTypeName())
				...
				if ("".equals(lastGroupTypeName)) {
					...
					group.setIndividualContactYN(contact
							.getIndividualContactYN());
		...
		listGroups.add(group);
		ApplicationContactGroupDTO temp[] = new ApplicationContactGroupDTO[listGroups
				.size()];
		for (int i = 0; i < temp.length; i++) {
	...
	public ComplianceControlStatusDTO[] getApplicationComplianceControlStatus(
		...
		if (null != contactsInput.getApplicationId()) {
			...
			List<ComplianceControlStatusDTO> listControls = ApplReposHbn
					.getComplianceControlStatus(tableId,
	...
	public HistoryViewDataDTO[] getApplicationHistory(
		...
		if (null != detailInput.getId() && 0 < detailInput.getId().longValue()) {
			...
			List<HistoryViewDataDTO> listHistory = AnwendungHbn
					.findApplicationHistory(detailInput.getTableId(),
							detailInput.getId());
